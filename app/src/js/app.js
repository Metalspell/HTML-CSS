...
experience.forEach(item => {
	...
	expirienceData += ` <h4 class="company-name mb-2">${item.company} - ${item.city} </i></h4>`;
	expirienceData += `<h5>${item.position}</h5>`;
	expirienceData += `<p>${item.description}</p>`;
	expirienceData += `<p>Link to work: <a href="${item.link}">${item.link}</a></p>`;
...
document.querySelector('.contacts-block').innerHTML += `<li><a href="tel:${data.phone}">Phone: ${data.phone}</a></li>`
document.querySelector('.contacts-block').innerHTML += `<li><a href="mailto:${data.email}">Mail: ${data.email}</a></li>`
document.querySelector('.contacts-block').innerHTML += `<li><a href="callto:${data.skype}">Skype: ${data.skype}</a></li>`
document.querySelector('.contacts-block').innerHTML += `<li><a href="${data.linkedin}">Linkedin</a></li>`
document.querySelector('.contacts-block').innerHTML += `<li><a href="${data.git}">My GitHub</a></li>`
